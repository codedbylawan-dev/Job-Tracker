...
function Dashboard() {
  ...
  const refresh = async () => { await Promise.all([fetchJobs(), fetchStats()]); };
  ...
  const statusCounts = ['Applied', 'Interview', 'Offer', 'Rejected'].reduce((acc, st) => {
    ...
    <div style={s.page}>
      ...
      <div style={s.container}>
        ...
        <div style={s.topBar}>
          <div>
            ...
            <p style={s.subheading}>{jobs.length} total application{jobs.length !== 1 ? 's' : ''}</p>
          </div>
          <div style={s.actions}>
        ...
        {showChart && jobs.length === 0 && (
          <div style={s.emptyAnalytics}>
            ...
            <p style={{ fontWeight: 600, color: '#374151' }}>Analytics will appear here</p>
            <p style={{ color: '#9CA3AF', fontSize: '14px' }}>Add your first job application to get started</p>
          </div>
        )}
            ...
            {statuses.map(st => (
              <button key={st} onClick={() => setFilter(st)} style={{
                ...
                {st !== 'All' && (
                  <span style={{
                    ...s.countBadge,
                    background: filter === st ? 'rgba(255,255,255,0.25)' : '#F3F4F6',
        ...
        {filtered.length === 0 ? (
          ...
          filtered.map(job => (
            <JobCard key={job.id} job={job} headers={headers} onUpdate={refresh} />
          ))
        )}
      </div>
      ...
      {showAdd && (
        <AddJob
          ...
          onAdd={() => { refresh(); setShowAdd(false); }}
        />
      )}
    </div>
  );
...
const s = {
  ...
  toggleBtn:      { padding: '9px 16px', background: '#EEF2FF', color: '#4338CA', border: 'none', borderRadius: '8px', cursor: 'pointer', fontWeight: 600, fontSize: '13px' },
  exportBtn:      { padding: '9px 16px', background: '#ECFDF5', color: '#065F46', border: 'none', borderRadius: '8px', cursor: 'pointer', fontWeight: 600, fontSize: '13px' },
  addBtn:         { padding: '9px 20px', background: 'linear-gradient(135deg, #1F4E79, #2E86AB)', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer', fontWeight: 700, fontSize: '14px' },
  emptyAnalytics: { background: 'white', borderRadius: '12px', padding: '40px', textAlign: 'center', marginBottom: '24px', boxShadow: '0 1px 6px rgba(0,0,0,0.07)' },
  controls:       { marginBottom: '20px' },
  search:         { width: '100%', padding: '13px 16px', border: '1.5px solid #E5E7EB', borderRadius: '10px', fontSize: '15px', marginBottom: '14px', background: 'white', outline: 'none' },
  filterRow:      { display: 'flex', gap: '8px', flexWrap: 'wrap' },
  filterBtn:      { padding: '7px 14px', borderRadius: '8px', border: '1.5px solid', cursor: 'pointer', fontWeight: 600, fontSize: '13px', display: 'flex', alignItems: 'center', gap: '6px', transition: 'all .15s' },
  countBadge:     { borderRadius: '10px', padding: '1px 7px', fontSize: '12px', fontWeight: 700 },
  empty:          { textAlign: 'center', padding: '70px 0', color: '#9CA3AF', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '8px' },
};
